...
class Windows(QDialog, mainUI.Ui_Dialog):
    ...
    def onLogSuccess(self, cookie):
        ...
        # The API session will be passed during QueryWorker instantiation, so this line is no longer needed.
        # apis[self.dictionaryComboBox.currentIndex()].session.cookies.update(self.selectedDict.session.cookies)
    ...
    def on_btnSync_clicked(self):
        ...
        try:
            ...
            if not askUser(f"{err}\nDeleting it would delete ALL its cards and notes!!! Continue?", defaultno=True):
                ...
                self.logHandler.flush()
                return
            if not askUser(f"[DANGEROUS ACTION!!!] Are you sure to delete model '{MODEL_NAMES[0]}' AND all its cards/notes???", defaultno=True):
